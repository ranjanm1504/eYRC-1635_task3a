...
class EnhancedDetector(Node):
    def __init__(self):
        ...
        # Simulation topic configuration
        ...
        self.get_logger().info("║  Enhanced Detector - Task 3B          ║")
    ...
    def publish_json_data(self):
        """Publish detected objects as JSON strings (Handles Empty States)"""

        # ----------------------------------------
        # 1. Handle Bad Fruits (Always publish list)
        # ----------------------------------------
        fruits_data = []

        # Only populate list if we have data
        ...
        # ALWAYS publish the message, even if the list is empty []
        msg_fruit = String()
        msg_fruit.data = json.dumps(fruits_data)
        self.fruit_positions_pub.publish(msg_fruit)

        # ----------------------------------------
        # 2. Handle Fertilizer (Always publish dict)
        # ----------------------------------------
        ...
        if self.last_fertilizer_position is not None:
            ...
            # NOT detected state
            fertilizer_data = {
                "detected": "false",  # STRING, not boolean
                "frame_id": f"{TEAM_ID}_fertilizer_1",
                "position": {
                    "x": 0.0,
                    "y": 0.0,
        ...
        # ALWAYS publish the fertilizer message
        msg_fert = String()
        msg_fert.data = json.dumps(fertilizer_data)
        self.fertilizer_info_pub.publish(msg_fert)
    ...
    def publish_tf(self, frame_id, x, y, z):
        """Publish TF transform - CALLED FROM DETECTION METHODS"""
        t = TransformStamped()
    ...
    def detect_fertilizer_aruco(self, image_disp):
        ...
        if ids is not None and len(ids) > 0:
            for i, marker_id in enumerate(ids.flatten()):
                ...
                # ===== PUBLISH TF HERE (in detection method) =====
                frame_id = f"{TEAM_ID}_fertilizer_1"
    ...
    def color_callback(self, msg):
        ...
        cv2.imshow("Enhanced Detection - Task 3B", image_disp)
        cv2.waitKey(1)
